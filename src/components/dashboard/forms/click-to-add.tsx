...
function ClickToAddInputs<T extends Detail = Detail>({
    ...
    //Funtion to handle changes in detail properties
    const handleDetailsChange = (index: number, property: string, value: string | number) => {
        // Update the details array with the new prperty value
    ...
    const handleRemove = (index: number) => {
        //We must at least keep one detail we can't delete if it's the only detail available
        if (details.length === 1) return;
        //Filter out the detail at the specified index
        const upadtedDetails = details.filter((_, i) => i !== index);
        setDetails(upadtedDetails); //Update the state with the filtered details
    }
                                    ...
                                    (
                                        <div className="flex gap-x-4">
                                            ...
                                            <span
                                                ...
                                                style={{ backgroundColor: detail.color as string }}
                                            />
                                        </div>
                                    )}

                                {/* Color picker */}
                                {colorPickerIndex === index && property === "color" && <SketchPicker
